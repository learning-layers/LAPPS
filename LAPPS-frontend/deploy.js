/* 
Deploy script for LAPPS-frontend
run with: npm run deploy

What it does:
- Scan index.html for js and css
- concat js by defined groups
- modify index.html to use merged js file names
- minify all js, css  and html files
- store all required files in a deploy directory
- create a gzip version for every resource
*/
(function(args) {
	...
	var devDirectory='app/'; //directory containing all files during development
	
	//define how to group js files
	var jsGroups=[{baseDirs:['core','components','shared'],groupFile:'js/lapps.js'},{baseDirs:['bower_components'],groupFile:'js/libs.js'}];
	//define which other files should be copied
	var copyPaths=['**/*.html','assets/img/**/*','assets/dummy/**/*','bower_components/bootstrap/dist/fonts/*'];
	...
	function minifyCSS(css){
		return new CleanCSS({keepSpecialComments: 0}).minify(css);
	}
		...
		for (var k=0; k<patterns.length;k++){
			(function(k){
				...
				glob(devDirectory+patterns[k], {}, function (err, files) {			
					...
					for(var i=0; i<files.length; i++){
						...
						if(!files[i].endsWith('index.html')){ //do not copy index.html
							if (files[i].endsWith('.html')){
								...
								fs.writeFileSync(targetPath, minifyHtml(fileContent), FILE_ENCODING);
					...
					if(k==patterns.length-1) //proceed with other steps when all files have been copied
						callback();
				});
			})(k);
		}
	...
	function gzipFilePure(src,dest){
		...
		fs.createReadStream(src).pipe(gzip).pipe(fs.createWriteStream(dest+'.gz'));
	}
	...
	function buildScripts(groups,scripts) {
		...
		for(var i=0;i<groups.length;i++){
			...
			concat({src:fileList, dest : groupFile});
		}
		...
		for(var i=0;i<tempCopy.length;i++){
			...
			if(!alreadyExisting)
			{
				...
				if(!tempCopy[i].isGrouped){//minify non-grouped js
					var fileContent=fs.readFileSync(devDirectory+tempCopy[i].file, FILE_ENCODING);
	...
	function insertScriptsInHtml(html,scripts){
		...
		html=html.substring(0,startPoint)+scriptBlock+html.substring(startPoint);
		return html;
	}
