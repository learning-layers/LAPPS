...
(function() {
      ...
      on: {
        error: function(response) {
          if (_this.url.substring(0, 4) !== 'http') {
            ...
            return _this.fail(response.status + ' : ' + response.statusText
                    + ' ' + _this.url);
          }
        },
  ...
  SwaggerApi.prototype.selfReflect = function() {
    ...
    if (this.apis == null) { return false; }
    _ref = this.apis;
    ...
    for (resource_name in _ref) {
      ...
      for (operation_name in _ref1) {
        ...
        for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
          parameter = _ref2[_i];
          log("  " + parameter.name + (parameter.required ? ' (required)' : '')
  ...
  var SwaggerResource = function(resourceObj, api) {
    ...
    this.path = this.api.resourcePath != null ? this.api.resourcePath
            : resourceObj.path;
    ...
    this.models = {};
    this.rawModels = {};
    this.useJQuery = (typeof api.useJQuery !== 'undefined' ? api.useJQuery
            : null);
    ...
    if ((resourceObj.apis != null) && (this.api.resourcePath != null)) {
      ...
      var obj = {
        ...
        on: {
          ...
          error: function(response) {
            return _this.api.fail("Unable to read api '" + _this.name
                    + "' from path " + _this.url + " (server returned "
                    + response.statusText + ")");
          }
        }
      };
  ...
  SwaggerResource.prototype.addApiDeclaration = function(response) {
    ...
    if ((response.basePath != null)
            && response.basePath.replace(/\s/g, '').length > 0)
      this.basePath = response.basePath.indexOf("http") === -1 ? this
              .getAbsoluteBasePath(response.basePath) : response.basePath;
    ...
    if (response.apis) {
      for (var i = 0; i < response.apis.length; i++) {
        ...
        this.addOperations(endpoint.path, endpoint.operations,
                response.consumes, response.produces);
      }
    }
  ...
  SwaggerResource.prototype.addOperations = function(resource_path, ops,
    ...
    if (ops) {
      ...
      for (var i = 0; i < ops.length; i++) {
        ...
        var op = new SwaggerOperation(o.nickname, resource_path, method,
                o.parameters, o.summary, o.notes, type, responseMessages, this,
                consumes, produces, o.authorizations, o.deprecated);
  ...
  var SwaggerModel = function(modelName, obj) {
    ...
    for (propertyName in obj.properties) {
      ...
      var prop = new SwaggerModelProperty(propertyName,
  ...
  SwaggerModel.prototype.setReferencedModels = function(allModels) {
    ...
    for (var i = 0; i < this.properties.length; i++) {
      ...
      else if ((property.refDataType != null)
  ...
  SwaggerModel.prototype.getMockSignature = function(modelsToIgnore) {
    ...
    var returnVal = classOpen + '<div>' + propertiesStr.join(',</div><div>')
            + '</div>' + classClose;
  ...
  var SwaggerModelProperty = function(name, obj) {
    ...
    this.isCollection = this.dataType
            && (this.dataType.toLowerCase() === 'array'
                    || this.dataType.toLowerCase() === 'list' || this.dataType
                    .toLowerCase() === 'set');
    ...
    this.dataTypeWithRef = this.refDataType != null ? (this.dataType + '['
            + this.refDataType + ']') : this.dataType;
  ...
  SwaggerModelProperty.prototype.getSampleValue = function(modelsToIgnore) {
    ...
    if ((this.refModel != null)
            && (modelsToIgnore.indexOf(this.refModel.name) === -1)) {
  ...
  SwaggerModelProperty.prototype.toString = function() {
    ...
    var str = '<span class="propName ' + req + '">' + this.name
            + '</span> (<span class="propType">' + this.dataTypeWithRef
            + '</span>';
    if (!this.required) {
    ...
    str += ')';
    if (this.values != null) {
      str += " = <span class='propVals'>['" + this.values.join("' or '")
              + "']</span>";
    }
    if (this.descr != null) {
  ...
  var SwaggerOperation = function(nickname, path, method, parameters, summary,
          notes, type, responseMessages, resource, consumes, produces,
          authorizations, deprecated) {
    var _this = this;

    var errors = [];
    this.nickname = (nickname || errors
            .push("SwaggerOperations must have a nickname."));
    this.path = (path || errors.push("SwaggerOperation " + nickname
            + " is missing path."));
    this.method = (method || errors.push("SwaggerOperation " + nickname
            + " is missing method."));
    ...
    if (typeof this.type !== 'undefined' && this.type === 'void')
      ...
      this.responseClassSignature = this.getSignature(this.type,
              this.resource.models);
      this.responseSampleJSON = this.getSampleJSON(this.type,
              this.resource.models);
    }
  ...
  SwaggerOperation.prototype.getSignature = function(type, models) {
    ...
    isPrimitive = ((listType != null) && models[listType])
            || (models[type] != null) ? false : true;
    if (isPrimitive) {
      return type;
    } else {
      if (listType != null) {
  ...
  SwaggerOperation.prototype.getSampleJSON = function(type, models) {
    ...
    isPrimitive = ((listType != null) && models[listType])
            || (models[type] != null) ? false : true;
    val = isPrimitive ? void 0 : (listType != null ? models[listType]
            .createJSONSample() : models[type].createJSONSample());
    if (val) {
  ...
  SwaggerOperation.prototype["do"] = function(args, opts, callback, error) {
    ...
    for (var i = 0; i < this.parameters.length; i++) {
      ...
      if (param.paramType === 'header') {
        ...
        if (args.body) { throw new Error(
                "Saw two body params in an API listing; expecting a max of one."); }
    ...
    req = new SwaggerRequest(this.method, this.urlify(args), params, opts,
            callback, error, this);
    if (opts.mock != null) {
      return req;
    } else {
    ...
    for (var i = 0; i < params.length; i++) {
      ...
      if (param.paramType === 'path') {
        if (args[param.name]) {
          ...
          var reg = new RegExp('\\{\\s*?' + param.name
                  + '.*?\\}(?=\\s*?(\\/?|$))', 'gi');
  ...
  SwaggerOperation.prototype.help = function() {
    ...
    for (var i = 0; i < params.length; i++) {
      var param = params[i];
      if (msg !== "") msg += "\n";
      msg += "* " + param.name + (param.required ? ' (required)' : '') + " - "
              + param.description;
    }
    return msg;
  };
  ...
  SwaggerOperation.prototype.formatXml = function(xml) {
    ...
    _fn = function(ln) {
      ...
      padding = ((function() {
        ...
        _results = [];
        for (j = _j = 0, _ref5 = indent; 0 <= _ref5 ? _j < _ref5 : _j > _ref5; j = 0 <= _ref5
                ? ++_j : --_j) {
          _results.push('  ');
        }
        return _results;
  ...
  var SwaggerRequest = function(type, url, params, opts, successCallback,
          errorCallback, operation, execution) {
    var _this = this;
    var errors = [];
    this.useJQuery = (typeof operation.resource.useJQuery !== 'undefined'
            ? operation.resource.useJQuery : null);
    this.type = (type || errors
            .push("SwaggerRequest type is required (get/post/put/delete/patch/options)."));
    this.url = (url || errors.push("SwaggerRequest url is required."));
    this.params = params;
    this.opts = opts;
    this.successCallback = (successCallback || errors
            .push("SwaggerRequest successCallback is required."));
    this.errorCallback = (errorCallback || errors
            .push("SwaggerRequest error callback is required."));
    this.operation = (operation || errors
            .push("SwaggerRequest operation is required."));
  ...
  SwaggerRequest.prototype.setHeaders = function(params, operation) {
    ...
    if (consumes && this.operation.consumes) {
      if (this.operation.consumes.indexOf(consumes) === -1) {
        log("server doesn't consume " + consumes + ", try "
   ...
   * magic. NOTE: when jQuery is available it will export both '$' and 'jQuery'
   * to the global space. Since we are using closures here we need to alias it
   * for internal use.
   */
  ...
  JQueryHttpClient.prototype.execute = function(obj) {
    ...
    obj.complete = function(response, textStatus, opts) {
      var headers = {}, headerArray = response.getAllResponseHeaders().split(
              "\n");
      ...
      for (var i = 0; i < headerArray.length; i++) {
        ...
        var name = toSplit.substring(0, separator).trim(), value = toSplit
                .substring(separator + 1).trim();
        headers[name] = value;
      }
  ...
  ShredHttpClient.prototype.registerProcessors = function(shred) {
    ...
    if (typeof window !== 'undefined') {
      this.content.registerProcessor(["application/json; charset=utf-8",
          "application/json", "json"], {
        parser: identity,
        stringify: toString
      });
    } else {
      this.Shred.registerProcessor(["application/json; charset=utf-8",
          "application/json", "json"], {
        parser: identity,
        stringify: toString
      });
  ...
  ShredHttpClient.prototype.execute = function(obj) {
    ...
    var res = {
      ...
      // Catch the Shred error raised when the request errors as it is made
      // (i.e. No Response is coming)
    ...
    if (this.type === "query") {
      ...
      if (typeof obj.headers[this.name] !== 'undefined') {
        if (typeof this.delimiter !== 'undefined')
          obj.headers[this.name] = obj.headers[this.name] + this.delimiter
                  + this.value;
      } else
  ...
  PasswordAuthorization.prototype.apply = function(obj, authorizations) {
    ...
    obj.headers["Authorization"] = "Basic "
            + base64encoder(this.username + ":" + this.password);
    return true;
  };
